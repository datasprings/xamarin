	...
	public class Platform : BindableObject, IPlatform, INavigation, IDisposable
	{
		...
		async Task<Page> INavigation.PopModalAsync(bool animated)
		{
			...
			if (_modals.Count >= 1 && controller != null)
				...
				await _renderer.DismissViewControllerAsync();
			DisposeModelAndChildrenRenderers(modal);
#else
		...
		internal void WillAppear()
		{
			...
			#if TODO
			_renderer.View.BackgroundColor = NSColor.White;
			_renderer.View.ContentMode = NSViewContentMode.Redraw;
			#endif
		...
		async Task PresentModal(Page modal, bool animated)
		{
			...
			if (_modals.Count > 1)
			{
				...
				var controller = GetRenderer(topPage) as NSViewController;
				if (controller != null)
				{
					controller.PresentViewController (wrapper, null);
					await Task.Delay(5);
					return;
				}
			}
			...
			_renderer.PresentViewController (wrapper, null);
			await Task.Delay(5);
		}
